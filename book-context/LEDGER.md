...
- 2026-08-14 — Sesión 12: hito CLI mínima (ADR-005) cumplido: nueva crate `liradb-cli` (binario `liradb`) con `demo` (4 consultas del brief con plan lógico + tabla + métricas por operador), `query "<LiraQL>"` y `help`; parseo manual sin clap (clap queda para cap. 31); `demo_graph()` promovido a API pública en vol2-liradb (único punto de verdad del grafo fixture); testable sin spawn (run(args, out, err) -> i32). 12 tests nuevos, ALL_GREEN (455 tests).
